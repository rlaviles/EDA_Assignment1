...
# load data and get the 2 dates we want to study, plot
...
# this is a summary of 4 plots
par(mfrow=c(2,2))

# plot data on top left (1,1)
plot(epUsed$DateTime,epUsed$GlobalActivePower,ylab='Global Active Power',xlab='',type='l')

# plot data on top right (1,2)
plot(epUsed$DateTime,epUsed$Voltage,xlab='datetime',ylab='Voltage',type='l')
...
plot(epUsed$DateTime,epUsed$GlobalReactivePower,xlab='datetime',ylab='Global_reactive_power',type='l')

# close device
x<-dev.off()
# remove transitory files
x<-unlink("data.txt")
x<-unlink("data.zip")